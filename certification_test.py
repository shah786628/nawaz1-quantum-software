...
def test_domain(domain, algorithm, data, description):
    ...
    try:
        ...
        if "routed_via" in rs or "dispatch" in rs or "bridge" in rs.lower():
            ...
            print("[OK] Algorithm routing confirmed")
...
N = 1024  # Example size; engine supports up to 2^53
...
test_domain("mathematics", "hhl", math_data, "Poisson equation RHS on 256x256 grid")
...
test_domain("finance", "qaoa", fin_data, "1024-asset portfolio factor model amplitudes")
...
test_domain("machine_learning", "vqe", ml_data, "QNN 1024-sample feature map with non-linear boundary")
...
test_domain("materials_science", "vqe", mat_data, "YBCO d-wave superconductor gap function (1024 k-points)")
...
test_domain("logistics", "qaoa", log_data, "256-city TSP distance matrix")
...
test_domain("turbulence_cfd", "vqe", turb_data, "Kolmogorov -5/3 turbulence energy spectrum (1024 modes)")
