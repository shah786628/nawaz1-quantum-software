...
# by the input_data array length (data.len().next_power_of_two()). The qubit count equals input_data length.
# The engine supports up to 2^53 amplitude values per request.
#
# This script uses Python to generate the amplitude data arrays (up to 2^53 elements) and
...
echo -e "${CYAN}"
echo "╔══════════════════════════════════════════════════════════════════════════╗"
echo "║  NAWAZ1 QUANTUM VQE ENGINE - Demo Runner (2^53-qubit scale)           ║"
...
# These demonstrate the API format. Full 2^53-qubit examples use Python.
# =============================================================================
echo -e "\n${YELLOW}[2/6] Quick API demos (curl, 1024-qubit inline examples)...${NC}"
echo -e "       (Full 2^53-qubit scale runs in Step 3 via Python)"
...
echo -e "\n${CYAN}  --- Pipeline: Hemoglobin (1024 amplitudes) ---${NC}"
curl -s -X POST "${BASE_URL}/quantum/pipeline/execute" \
    "${HEADERS[@]}" \
    -d "{\"domain\":\"chemistry\",\"molecule\":\"hemoglobin\",\"atoms\":8738,\"input_data\":${CHEM_DATA}}" | \
    python3 -m json.tool 2>/dev/null || echo "(raw output above)"

echo -e "\n${CYAN}  --- Multidimensional Query (1024 data points) ---${NC}"
curl -s -X POST "${BASE_URL}/multidimensional/query" \
    "${HEADERS[@]}" \
    -d '{"type":"range","dimensions":3,"data_points":1024,"bounds":{"min":[0,0,0],"max":[1,1,1]}}' | \
    python3 -m json.tool 2>/dev/null || echo "(raw output above)"

# =============================================================================
# Step 3: Run Python quantum examples (FULL 2^53-qubit scale, all 15 domains)
# =============================================================================
echo -e "\n${YELLOW}[3/6] Running FULL 2^53-qubit quantum examples (Python)...${NC}"
echo -e "       Each domain sends N amplitude values; engine allocates qubits accordingly (up to 2^53)"
if command -v python3 &>/dev/null; then
    python3 quantum_usage_examples.py 2>&1 || echo -e "${RED}  Some examples failed (server may not support all domains)${NC}"
else
    echo -e "${RED}  Python3 not found. Install Python 3.8+ with numpy to run 2^53-qubit examples.${NC}"
fi
...
echo "║  - Python:     1024 data points ? 1024 qubits (example scale, supports up to 2^53)           ║"
echo "║                                                                        ║"
echo "║  Domains tested: 15 (chemistry, physics, finance, materials,           ║"
echo "║    biomolecules, ML, logistics, math, error_mitigation,                ║"
