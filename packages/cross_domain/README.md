...
def amplitudes_from_result(prev, key="amplitudes", size=1024):
    """Lift a previous stage's amplitude payload into a normalized N-length vector."""
...
1. **`amplitudes_from_result(...)`** — turn the previous stage's vector output into the next stage's `input_data` (N floats for the engine (supports up to 2^53)).
...
mol_amps = normalize(np.random.default_rng(1).normal(size=1024)  # Example; engine supports up to 2^53).tolist()
...
flow_in = normalize(np.random.default_rng(2).normal(size=1024)  # Example; engine supports up to 2^53).tolist()
...
ocean_in = normalize(np.random.default_rng(3).normal(size=1024)  # Example; engine supports up to 2^53).tolist()
...
```json
{
  "domain": "physics",
  "algorithm": "vqe",
  "input_data": "[ N amplitudes encoding the neutron density field over the 17x17x264 fuel-assembly lattice ]",
  "config": {
...
```json
{
  ...
  "algorithm": "hhl",
  "input_data": "[ N amplitudes piped from physics flux_amplitudes ]",
  "config": {
...
```json
{
  ...
  "algorithm": "vqe",
  "input_data": "[ N amplitudes piped from heat_transfer temperature_amplitudes ]",
  "config": {
...
```json
{
  ...
  "algorithm": "vqe",
  "input_data": "[ N amplitudes piped from materials_science stress_amplitudes ]",
  "config": {
    "sub_module":      "navier_stokes",
...
core_amps = normalize(np.random.default_rng(4).normal(size=1024)  # Example; engine supports up to 2^53).tolist()
...
```json
{
  ...
  "algorithm": "vqe",
  "input_data": "[ N amplitudes encoding a downsampled LiDAR point cloud of the 200m forward scene ]",
  "config": {
...
```json
{
  "domain": "physics",
  "algorithm": "vqe",
  "input_data": "[ N trajectory amplitudes piped from ML detection_amplitudes ]",
  "config": {
...
```json
{
  "domain": "logistics",
  "algorithm": "qaoa",
  "input_data": "[ N amplitudes piped from physics trajectory_amplitudes ]",
  "config": {
...
```json
{
  "domain": "real_time",
  "algorithm": "vqe",
  "input_data": "[ N amplitudes piped from logistics route_amplitudes ]",
  "config": {
...
lidar_amps = normalize(np.random.default_rng(5).normal(size=1024)  # Example; engine supports up to 2^53).tolist()
...
```json
{
  "domain": "finance",
  "algorithm": "qaoa",
  "input_data": "[ N amplitudes encoding a 12-currency, 8-commodity joint return distribution ]",
  "config": {
    "sub_module":         "portfolio",
...
```json
{
  "domain": "logistics",
  "algorithm": "qaoa",
  "input_data": "[ N amplitudes piped from finance hedge_amplitudes ]",
  "config": {
...
```json
{
  "domain": "machine_learning",
  "algorithm": "vqe",
  "input_data": "[ N amplitudes piped from logistics flow_amplitudes ]",
  "config": {
    "sub_module":      "quantum_kernel",
...
```json
{
  "domain": "mathematics",
  "algorithm": "qaoa",
  "input_data": "[ N amplitudes piped from ML forecast_amplitudes ]",
  "config": {
    "sub_module":   "quantum_optimization_theory",
...
fin_amps = normalize(np.random.default_rng(6).normal(size=1024)  # Example; engine supports up to 2^53).tolist()
...
```json
{
  "domain": "chemistry",
  "algorithm": "vqe",
  "input_data": "[ N amplitudes encoding the candidate's electronic Hamiltonian ]",
  "config": {
    "sub_module":  "vqe_chemistry",
    "task":        "ground_state_energy",
...
```json
{
  "domain": "biomolecules",
  "algorithm": "vqe",
  "input_data": "[ N amplitudes piped from chemistry wavefunction_amplitudes ]",
  "config": {
    "sub_module":          "drug_discovery",
    "task":                "binding_affinity",
    "target_pdb":          "6LU7",
...
```json
{
  "domain": "materials_science",
  "algorithm": "vqe",
  "input_data": "[ N amplitudes piped from biology binding_amplitudes ]",
  "config": {
...
```json
{
  "domain": "finance",
  "algorithm": "qaoa",
  "input_data": "[ N amplitudes piped from materials_science stability_amplitudes ]",
  "config": {
    "sub_module":     "portfolio",
    "task":           "drug_pipeline_valuation",
...
```json
{
  "domain": "machine_learning",
  "algorithm": "vqe",
  "input_data": "[ N amplitudes piped from finance valuation_amplitudes ]",
  "config": {
    "sub_module":  "quantum_kernel",
...
mol_amps = normalize(np.random.default_rng(7).normal(size=1024)  # Example; engine supports up to 2^53).tolist()
chem = execute("chemistry", "vqe", mol_amps, {
...
seed = normalize(np.random.default_rng(8).normal(size=1024)  # Example; engine supports up to 2^53).tolist()
...
# Converge — concatenate all three result vectors and pad to desired size
...
fused = normalize(np.pad(fused, (0, max(0, 1024 - fused.size)))[:1024]).tolist()
...
**When:** the three upstream domains are *independent given* the seed amplitudes. Fan-out cuts wall-clock latency by ~3× because the VQE engine calls run concurrently against the shared 2^53-qubit substrate.
...
amps    = normalize(np.random.default_rng(9).normal(size=1024)  # Example; engine supports up to 2^53).tolist()
prev_k  = float("inf")
for it in range(20):
...
seeds = [normalize(np.random.default_rng(s).normal(size=1024)  # Example; engine supports up to 2^53).tolist() for s in range(4)]
candidates = []
...
seed  = normalize(np.random.default_rng(10).normal(size=1024)  # Example; engine supports up to 2^53).tolist()
...
- **Watch the qubit budget.** Every stage allocates `next_power_of_two(len(input_data))` qubits. Pad/truncate to the desired width (power of 2, up to 2^53) at every hand-off.
...
```json
{
  "problem": "multi_physics_pipeline",
  "config": {
    "num_qubits": 1024,
    "optimizer": "SPSA",
    "max_iterations": 100
...
- `"multi_physics_pipeline"` — Chain multiple quantum domains into a single workflow
...
|---|---|---|
| Chained Hamiltonians | Sequential Hamiltonians from multiple domains | Multi-physics coupling |
| Domain-Specific | Per-stage Hamiltonian from target domain | Individual pipeline stages |
| Composite | Combined Hamiltonians for coupled problems | Convergence loops |
...
```json
{
  "hamiltonian": {
    "type": "chained",
    "parameters": {
      "stages": ["chemistry", "biology", "finance"],
      "coupling": "amplitude_transfer"
    }
  }
}
```
...
### Encoding Options
- **Jordan-Wigner**: Fermion-to-qubit mapping (default for chemistry/physics stages)
- **Bravyi-Kitaev**: Reduced gate depth for large systems
- **Direct Encoding**: For combinatorial optimization stages
