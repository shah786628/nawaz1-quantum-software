...
The Fluid Mechanics package provides quantum-accelerated computational fluid dynamics through the unified VQE engine at 2^53-qubit scale. It encompasses **6 specialized sub-modules** covering the full spectrum of CFD — from incompressible Navier-Stokes to quantum-native linear solvers for fluid systems.
...
|---|-----------|------------|
| 1 | Navier-Stokes  | Core Flow Solver |
| 2 | Turbulence  | Turbulence Modeling |
| 3 | Compressible  | Compressible Flow |
| 4 | Multiphase  | Interface Tracking |
| 5 | Heat Transfer  | Thermal Simulation |
| 6 | Quantum CFD  | Quantum Linear Solvers |
...
Core incompressible and compressible Navier-Stokes solver supporting multiple flow regimes with spatial discretization schemes and finite volume method integration at 2^53-qubit scale via the VQE engine.
...
Comprehensive turbulence modeling framework providing DNS, RANS, LES, and hybrid methods for resolving or modeling turbulent flow structures at all relevant scales via the VQE engine at 2^53-qubit scale.
...
Compressible flow solvers for Euler equations and compressible Navier-Stokes with advanced flux schemes for shock-capturing and high-speed flow simulation at 2^53-qubit scale via the VQE engine.
...
Multiphase flow simulation with interface tracking and surface tension modeling for immiscible fluid systems at 2^53-qubit scale via the VQE engine.
...
Heat transfer simulation with multiple modes including conduction, convection, conjugate heat transfer, and radiation at 2^53-qubit scale via the VQE engine.
...
Quantum CFD integration providing quantum linear solvers for fluid mechanics systems, quantum error mitigation, and quantum circuit configuration for CFD workloads at 2^53-qubit scale via the VQE engine.
...
- **Maximum grid:** 256×256 (cells per 2D slice, up to 2^53 total)
...
# Generate amplitude-encoded flow field state
...
- `"navier_stokes"` — Incompressible and compressible flow solutions
- `"flow_simulation"` — General fluid dynamics simulation
...
|---|---|---|
| Lattice Boltzmann | Kinetic theory discretization | Mesoscale flow simulation |
| Navier-Stokes Discretized | FVM/FDM/FEM spatial discretization | General CFD |
| Euler Equations | Inviscid compressible flow | High-speed aerodynamics |
| Quantum Linear System | HHL-based pressure Poisson solver | Quantum-accelerated CFD |

### Configuration
```json
{
  "hamiltonian": {
    "type": "navier_stokes_discretized",
    "parameters": {
      "regime": "incompressible",
      "reynolds_number": 100000,
      "grid": [256, 256],
...
### Encoding Options
- **Jordan-Wigner**: For quantum lattice Boltzmann formulations
- **Bravyi-Kitaev**: Reduced gate depth for large flow systems
- **Direct Encoding**: For discretized linear systems (HHL-based solvers)
